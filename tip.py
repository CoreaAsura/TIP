...
st.set_page_config(page_title="우주 낙하 분석기", layout="wide")
st.title("☄️ 우주 낙하 분석기 (ver. improved)")
st.markdown(f"🕒 현재 시간 (KST): `{kst_now.strftime('%Y-%m-%d %H:%M:%S')}`")
...
if st.button("📂 TLE 파일 불러오기"):
    if os.path.exists(TLE_FILE):
        ...
        st.success(f"🛰️ 총 `{len(tle_sets)}` 개 TLE 세트 불러오기 완료")
    else:
...
if st.button("☄️ 분석 시작"):
    ...
    if not tle_sets:
        ...
        if result_objects:
            ...
            st.download_button("📂 분석 결과 CSV 다운로드", data=csv, file_name="decay_result.csv", mime="text/csv")
        else:
            st.info("✅ 해당 반경 내 낙하 물체 없음")
