...
if decay_data:
    for obj in decay_data[:20]:  # 샘플 표시
        name = obj.get("satname", "N/A")
        country = obj.get("country", "N/A")
        date = obj.get("decay_date", "N/A")
        st.markdown(f"- **{name}** | 국가: {country} | 낙하예상일: {date}")
...
st.markdown("---")
st.caption("📡 데이터 출처: space-track.org | 분석 기반: TIP 메시지 & Satellite Decay API")
